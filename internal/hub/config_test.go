...
func TestValidateRejectsInvalidInventory(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "option target", mutate: func(i *Inventory) { i.Hosts[0].Target = "-oProxyCommand=bad" }, want: "must not begin with a hyphen"},
		...
		{name: "option unit", mutate: func(i *Inventory) { i.Hosts[0].Services[0].Unit = "-Hremote.service" }, want: "unit is invalid"},
